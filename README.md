...
Omar Hassanein: I implemented several defined functions in the Streamlit UI application to enable sentiment analysis and image generation based on user input. These functions processed the input tweet text, performed sentiment analysis using both pre-trained and custom models, and generated images based on the tweet content using the DALL-E-3 model. This allowed users to input their tweet text, trigger sentiment analysis, and generate images seamlessly within the Streamlit interface, enhancing the overall user experience of the Transfors AI project.

Raymond Conley:  In this project, I contributed to researching and selecting tweet and emoji datasets essential for developing a sentiment analysis model to categorize tweets as positive, neutral, or negative. Although we ultimately did not include the emoji feature in the final implementation, my tasks extended to executing, troubleshooting, and testing the sentiment analysis models. I also supported the team in achieving our project objectives, such as implementing and optimizing the model, documenting our processes on GitHub, and preparing the presentation materials. My comprehensive involvement helped achieve the project's success from conception through to its final presentation.
...
Bug Report: Preprocessing User Input Tweets

Description:
Currently, there is an issue with preprocessing user input tweets to match the format expected by our logistic regression model. The model requires input in a specific tokenized and padded format, but we are encountering difficulties in converting user input text to this format.
